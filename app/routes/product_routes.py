...
def create_product(product: ProductCreate, conn=Depends(get_connection)):
    try:
        return ProductService(conn).create(
...
def update_product(product_id: int, product: ProductUpdate, conn=Depends(get_connection)):
    try:
